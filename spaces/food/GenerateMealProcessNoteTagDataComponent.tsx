...
function EditorComponent({ noteTag, note, notu }: NoteTagDataComponentProps) {
    ...
    const data = new GenerateMealProcessData(noteTag);
    ...
    function onSaveMealToSpaceChange(newValue: number) {
        data.saveMealToSpaceId = newValue;
        manualRefresh();
    }
    ...
    return (
        <YStack>
            ...
            <XStack style={{alignItems: 'center'}}>
                <Label width={labelWidth}>Save Meal To</Label>
                <NotuSelect options={selectOptions}
                            value={data.saveMealToSpaceId}
                            onValueChange={onSaveMealToSpaceChange} />
            </XStack>
        </YStack>
    )
}