...
function EditorComponent({ noteTag, note, notu }: NoteTagDataComponentProps) {
    ...
    const data = new CelebrationEventsProcessData(noteTag);
    ...
    function onSavePlanTasksToSpaceChange(newValue: number) {
        data.savePlanTasksToSpaceId = newValue;
        manualRefresh();
    }
    ...
    return (
        <YStack>
            ...
            <XStack style={{alignItems: 'center'}}>
                <Label width={labelWidth}>Save Plan Tasks To</Label>
                <NotuSelect options={selectOptions}
                            value={data.savePlanTasksToSpaceId}
                            onValueChange={onSavePlanTasksToSpaceChange} />
            </XStack>
        </YStack>
    )
}