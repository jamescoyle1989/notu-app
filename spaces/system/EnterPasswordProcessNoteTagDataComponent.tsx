...
import { EnterPasswordProcessData } from "./EnterPasswordProcessNoteTagData";
import defs from "./SystemSpaceDefs";

export default class EnterPasswordProcessNoteTagDataComponentFactory implements NoteTagDataComponentFactory {
    ...
    isForNoteTag(note: Note, tag: Tag): boolean {
        return tag.space.internalName == defs.internalName &&
            tag.name == defs.enterPasswordProcess;
    }
}


function EditorComponent({ noteTag, note }: NoteTagDataComponentProps) {
    const data = new EnterPasswordProcessData(noteTag);
